    ...
    <article className="bg-[color:var(--color-paper)]">
      ...
      <div className="bg-[color:var(--color-ink)] text-[color:var(--color-paper)]">
        <div className="mx-auto max-w-4xl px-4 py-14 sm:px-6 md:py-20">
          <Link
            href="/#soins"
            className="inline-flex items-center gap-1.5 font-mono text-[length:var(--step--1)] uppercase tracking-wide text-[color:color-mix(in_srgb,var(--color-paper)_70%,transparent)] hover:text-[color:var(--color-tape)]"
          >
            <ArrowRight className={`size-4 ${isRtl ? "" : "rotate-180"}`} aria-hidden="true" />
            {t("allSoins")}
          </Link>
          <p className="mt-6 font-mono text-[length:var(--step--1)] uppercase tracking-[0.2em] text-[color:var(--color-tape)]">
            {site.area[locale]}
          </p>
          <h1 className="mt-3 text-[length:var(--step-4)] font-extrabold">{soin.title[locale]}</h1>
      ...
      <div className="mx-auto grid max-w-4xl gap-12 px-4 py-14 sm:px-6 md:grid-cols-2 md:py-20">
        ...
        <section>
          <h2 className="font-mono text-[length:var(--step--1)] uppercase tracking-widest text-[color:var(--color-slate)]">
            {t("conditionsTitle")}
          </h2>
          <ul className="mt-4 space-y-3">
            {soin.conditions.map((c) => (
              <li key={c.fr} className="flex items-start gap-3">
                <Check className="mt-0.5 size-4 shrink-0 text-[color:var(--color-tape-ink)]" aria-hidden="true" />
          ...
          <ul className="mt-4 space-y-3">
            {soin.approach.map((a) => (
              <li key={a.fr} className="flex items-start gap-3">
                <span className="mt-2 size-1.5 shrink-0 rounded-full bg-[color:var(--color-tape-ink)]" aria-hidden="true" />
